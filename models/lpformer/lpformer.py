...
class LPFormer(nn.Module):
    ...
    def forward(self, points_input, center_position):
        ...
        # gal
        gal_p = self.gal(local_points)
        gal_p = F.avg_pool2d(gal_p, (32, 1))  # 沿着 32 维进行池化
        gal_p = gal_p.squeeze(2)
        points_feat = torch.cat([gal_p , local_points], dim = -1)
        
        # # # no gal
        # # points_feat = torch.cat([points_input, local_points], dim = -1) #error
        # points_feat = torch.cat([local_points, points_input], dim = -1) 
