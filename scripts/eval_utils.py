...
def joint_accuracy(predicted, target, threshold):
    ...
    # Calculate PCK per joint (percentage of correct keypoints)
    joint_pck = (joint_wise_correct_points / n_frames) * 100  # PCK for each joint

    # Calculate mAP correctly
    # Average accuracy per frame, where frame_wise_correct_points is the correct keypoints per frame
    frame_accuracy = frame_wise_correct_points.float() / 15
    mean_ap = torch.mean(frame_accuracy) * 100  # Average mAP across frames

    # Debug the output

    print(f"PCK per joint: {joint_pck}")
    print(f"Mean AP (corrected): {mean_ap}")

    return joint_pck, mean_ap
